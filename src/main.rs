...
fn config_path_default() -> String {
    dirs::home_dir().unwrap().join(".mailsitter").join("email.yaml").to_string_lossy().to_string()
}
...
async fn login_ddep(u: String, config: email::EmailConfig) -> Result<(), Box<dyn Error>>{
    ...
    let emails = config.fetch_until("NOT OR OR NOT FROM support@duck.com NOT BODY \"one-time passphrase\" SEEN", 10, 0.5).await?;
    if emails.len() > 0 {
...
async fn parse_cmd(cmd: Commands) -> Result<(), Box<dyn Error>> {
    match cmd {
        Commands::Init { email, pwd, smtp, path, username, .. } => {
	    ...
	    // A AND B = NOT(NOT A OR NOT B)
            config.fetch_email("NOT OR OR NOT FROM support@duck.com NOT BODY \"one-time passphrase\" SEEN")?;
        },
	#[cfg(feature="ddep")]
		...
		let stdin = io::stdin();
		let input = {
		    stdin.lock().lines().next().unwrap_or_else(|| Ok(String::from("")))?
		};
...
async fn main() -> Result<(), Box<dyn Error>> {
	...
	klask::run_derived::<Args, _>(klask::Settings::default(), |o| {
            println!("{:#?}", o)
	});
    }
