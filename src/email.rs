...
impl <'a> From<mailparse::ParsedMail<'a>> for Email {
	...
	let body = if email.ctype.mimetype.starts_with("text/plain") {
	    ...
	    email.subparts.iter().fold("".to_string(), |a, b| {
...
impl EmailConfig {
    ...
    pub async fn fetch_until(&self, filter: &str, timeout: u64, period: f64) -> Result<Vec<Email>, Box<dyn Error>> {
	...
	loop {
		...
		return Err(Box::new(TimeoutError("Timeout, cannot find new email".to_string())))
            }
